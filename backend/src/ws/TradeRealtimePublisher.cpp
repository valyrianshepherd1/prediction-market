...
namespace pm::ws {
    ...
    bool TradeRealtimePublisher::supports(RealtimePublishKind kind) const noexcept {
        return kind == RealtimePublishKind::TradeExecution;
    }

    void TradeRealtimePublisher::publish(const RealtimePublishRequest &request) const {
        const auto *tradeRequest = std::get_if<TradeExecutionRequest>(&request);
        if (!tradeRequest) {
            return;
        }

        const auto &trade = tradeRequest->trade;
        ...
        publishOrderSnapshotById(tradeRequest->db, trade.maker_order_id, "matched");
        if (trade.taker_order_id != trade.maker_order_id) {
            publishOrderSnapshotById(tradeRequest->db, trade.taker_order_id, "matched");
        }

        publishRealtime(WalletSnapshotRequest{tradeRequest->db, trade.maker_user_id});
        if (trade.taker_user_id != trade.maker_user_id) {
            publishRealtime(WalletSnapshotRequest{tradeRequest->db, trade.taker_user_id});
        }

        publishPositionSnapshot(tradeRequest->db, trade.maker_user_id, trade.outcome_id);
        if (trade.taker_user_id != trade.maker_user_id) {
            publishPositionSnapshot(tradeRequest->db, trade.taker_user_id, trade.outcome_id);
        }
    }
