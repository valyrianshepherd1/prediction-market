...
void TradeRepository::listTradesByOutcome(const std::string &outcomeId,
                                          int limit,
                                          int offset,
    ...
    static constexpr std::string_view kSql =
            ...
            "  maker_user_id::text AS maker_user_id, "
            "  taker_user_id::text AS taker_user_id, "
...
void TradeRepository::listTradesByUser(const std::string &userId,
                                       int limit,
                                       int offset,
                                       std::function<void(std::vector<TradeRow>)> onOk,
                                       std::function<void(const DrogonDbException &)> onErr) const {
    ...
    static constexpr std::string_view kSql =
            ...
            "  created_at::text AS created_at "
            "FROM trades "
            "WHERE maker_user_id = $1::uuid OR taker_user_id = $1::uuid "
            "ORDER BY created_at DESC "
            "LIMIT $2 OFFSET $3";
